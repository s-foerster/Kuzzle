...
import {
  ...
  CELL_DARK,
  CELL_LIGHT,
} from "../../algorithms/lumizle/rules.js";
...
const allCells = computed(() => {
  ...
  for (let r = 0; r < props.gridSize; r++)
    for (let c = 0; c < props.gridSize; c++) cells.push({ r, c });
  return cells;
});
...
function handleTouchMove(e) {
  ...
  if (!touchHasDragged.value && touchStartCell.value) {
    emit(
      "cell-drag",
      touchStartCell.value.r,
      touchStartCell.value.c,
      touchDragTarget.value,
    );
...
.lumizle-grid {
  ...
  background-color: #b1b5c4; /* fond plus contrasté pour mieux détacher le blanc */
  position: relative;
  touch-action: none;
}
