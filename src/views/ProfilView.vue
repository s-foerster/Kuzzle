  ...
  <main class="app-main">
    ...
    <div v-else class="profil-container">
      ...
      <Transition name="fade">
        <div
          v-if="paymentSuccess"
          class="profil-section profil-section--success"
        >
      ...
      <div
        class="profil-section profil-section--premium"
        :class="{ 'profil-section--premium-active': authStore.isPremium }"
      >
      ...
      <div class="profil-section">
        ...
        <div class="username-form">
          ...
          <button
            ...
            :disabled="
              savingUsername ||
              !newUsername.trim() ||
              newUsername.trim() === authStore.username ||
              !!usernameFormatError
            "
          >
        ...
        <p class="format-hint">
          3-20 caractères · lettres minuscules, chiffres et _
        </p>
        <p v-if="usernameFormatError" class="field-error">
          {{ usernameFormatError }}
        </p>
        <p v-else-if="usernameError" class="field-error">{{ usernameError }}</p>
        <p v-else-if="usernameSuccess" class="field-success">
          Pseudo mis à jour !
        </p>
      </div>
        ...
        >
          <div class="delete-modal" role="dialog" aria-modal="true">
            ...
            <p class="delete-modal-body">
              ...
              statistiques et tout votre historique seront définitivement
              supprimés.
            </p>
            <p class="delete-modal-confirm-hint">
              Tapez votre adresse e-mail
              <strong class="confirm-username">{{
            ...
            <div class="delete-modal-actions">
              ...
              <button
                ...
                @click="deleteAccount"
                :disabled="
                  deleteConfirmInput !== (authStore.user?.email || '') ||
                  deletingAccount
                "
              >
                {{
...
const visibleResults = computed(() =>
  filteredResults.value.slice(0, displayCount.value),
);
const hasMore = computed(
  () => displayCount.value < filteredResults.value.length,
);
...
function calcStreak(gameType) {
  ...
  for (let i = 1; i < dateStrs.length; i++) {
    const diff = Math.round(
      (new Date(dateStrs[i]) - new Date(dateStrs[i - 1])) / 86400000,
    );
    if (diff === 1) {
      run++;
  ...
  if (last === todayStr || last === yestStr) {
    ...
    for (let i = dateStrs.length - 2; i >= 0; i--) {
      const diff = Math.round(
        (new Date(dateStrs[i + 1]) - new Date(dateStrs[i])) / 86400000,
      );
      if (diff === 1) current++;
      else break;
    }
...
async function deleteAccount() {
  ...
  if (deleteConfirmInput.value !== confirmTarget || deletingAccount.value)
    return;
