<template>
  <main class="app-main">
    ...
    <div v-else-if="lumizleError" class="error-message">
      ...
      <button @click="initDailyPuzzle()" class="btn btn-primary">Réessayer</button>
    </div>
    ...
    <div v-else-if="lumizlePuzzle" class="game-container">
      ...
      <div class="timer-row">
        <div
          class="timer-display"
          :class="{ 'timer-paused': lumizleIsPaused, 'timer-won': lumizleIsWon }"
        >
      ...
      <Transition name="modal-fade">
        <div v-if="lumizleIsPaused" class="pause-modal" @click.self="lumizleTogglePause">
          <div class="pause-content">
            <h2>⏸ Pause</h2>
            <div class="pause-timer">{{ lumizleFormattedTime }}</div>
            <button @click="lumizleTogglePause" class="btn btn-primary">▶ Reprendre</button>
          </div>
        </div>
      </Transition>

      <!-- Grille + Règles -->
      <div class="lumizle-play-area" :class="{ 'grid-blurred': lumizleIsPaused }">
      ...
      <div class="actions">
        ...
        >&#x21A9;</button>
        <button @click="lumizleResetGameState" class="btn btn-secondary">🔄</button>
      </div>
    </div>
  </main>
</template>
...
const {
  puzzle: lumizlePuzzle, gameState: lumizleGameState,
  isWon: lumizleIsWon, isLoading: lumizleIsLoading, error: lumizleError,
  formattedTime: lumizleFormattedTime, isTimerStarted: lumizleIsTimerStarted,
  isPaused: lumizleIsPaused, togglePause: lumizleTogglePause,
  undoHistory: lumizleUndoHistory, undo: lumizleUndo,
  fixedCells: lumizleFixedCells, violatingCells: lumizleViolatingCells,
  handleCellClick: lumizleHandleCellClick, handleCellDrag: lumizleHandleCellDrag,
  resetGameState: lumizleResetGameState,
  initPuzzle: lumizleInitPuzzle, initPracticePuzzle: lumizleInitPracticePuzzle,
  markCompleted: lumizleMarkCompleted, currentDate: lumizleCurrentDate,
...
watch(lumizleIsWon, async (won) => {
  ...
  if (!lumizleCompletedLevels.value.includes(id)) {
    ...
    localStorage.setItem('lumizle-completed-levels', JSON.stringify(lumizleCompletedLevels.value))
  }

  // Supabase (si connecté et puzzle quotidien)
  const isPractice = id.startsWith('easy_') || id.startsWith('medium_') || id.startsWith('hard_')
...
.timer-row {
  ...
  justify-content: center; gap: 0.75rem; margin-bottom: 1rem;
}
.timer-display {
...
.timer-display.timer-won {
  color: var(--color-success); border-color: var(--color-success); background: var(--color-success-light);
}
@keyframes blink { 0%, 100% { opacity: 1; } 50% { opacity: 0.65; } }

.btn-pause {
  padding: 0.5rem 1.1rem; border: 2px solid var(--color-primary);
...
.victory-card {
  width: 100%; background: var(--gradient-success); color: white;
...
.lumizle-grid-area  { flex: 1 1 0; min-width: 0; }
.lumizle-rules-area { flex: 0 0 200px; min-width: 160px; }

.actions {
  display: flex; justify-content: center;
  gap: 0.75rem; flex-wrap: wrap; margin-top: 0.75rem; width: 100%;
}
.btn {
...
.btn-secondary { background: var(--color-bg-muted); color: var(--color-text-soft); border: 1.5px solid var(--color-border); }
.btn-secondary:hover:not(:disabled) { background: var(--color-border); color: var(--color-text); transform: translateY(-1px); }

@media (max-width: 680px) {
  .lumizle-play-area { flex-direction: column; align-items: stretch; }
  .lumizle-rules-area { flex: none; }
}
